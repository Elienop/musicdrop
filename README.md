...
- **beets config** — viewer + writable editor, with advisory notices for import keys MusicDrop forces (a saved value that only affects CLI runs is flagged, not silently accepted). `import.delete` is one of them: MusicDrop never removes a source, so your downloads survive every import.

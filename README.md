...
- `MUSICDROP_TRASH_DIR` (default `<beets dir>/trash`) — where deleted albums wait. Put it inside the music library (`/music/.trash`) to keep deletes on one disk. Refused: a Trash that IS or CONTAINS the music library, the beets data dir, the origin store or another app folder — inside them is fine. A refusal stops startup with one `ERROR` line and answers `503` on the Trash routes. Spell it through the same mount as `directory:`.
- `MUSICDROP_TRASH_ORIGINS_DIR` (default `<beets dir>/trash-origins`) — the restore records (see **Backup & restore**); needs a folder of its own: not inside, on top of or around the music library, the beets dir or the Trash.

...
- `MUSICDROP_TRASH_DIR` (default `<beets dir>/trash`) — where deleted albums wait until you empty the Trash. It **may** point inside the music library, and that is the reason to set it: `MUSICDROP_TRASH_DIR=/music/.trash` keeps deletes on the same volume, so they are instant renames instead of copies onto the small `/data` disk. Plex may still list a dot-folder — Plex's docs don't say those are skipped — so add a line naming it to a `.plexignore` in the library root if it turns up. What is refused is a Trash that **is** the music library or the beets data dir, or that **contains** either. The startup check refuses to boot and writes one `ERROR` line naming the setting, both resolved paths, what would have been deleted and how to fix it; the same check runs again on every delete, Empty Trash, duplicate resolve, Reorganize and on the Trash listing itself, which answer `503` with that sentence when a symlink has moved under a running process. **Settings → Trash** prints it under “Couldn’t load Trash.”, so the screen that cannot list the folder is also the one that says which setting to move. `MUSICDROP_TRASH_ORIGINS_DIR` (the restore records — see **Backup & restore**) needs a directory of its own: it is refused anywhere under the music library or above it, at or above the beets data dir, and overlapping the Trash in either direction — including being the Trash. The default sibling `<beets dir>/trash-origins` satisfies all of those. Two more pairs are refused for the same reason, and both are set in `config.yaml` rather than by env var: `MUSICDROP_BEETS_DIR` and beets' `directory:` may not nest **in either direction** (the shipped `/data` and `/music` are separate trees, which is the shape to keep), and beets' `library:` may not sit in the Trash or in the origin store. Those two are reported as a red row on the offending line in **Settings → Beets**, and Save and Apply are refused with the same sentence.

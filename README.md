...
- `MUSICDROP_TRASH_DIR` (default `<beets dir>/trash`) — where deleted albums wait. Put it inside the music library (`/music/.trash`) to keep deletes on one disk; `.plexignore` it if Plex lists the dot-folder. Refused: a Trash overlapping the music library, the beets data dir or database, the origin store, or another app folder. A refusal stops startup with one `ERROR` line, answers `503` on the Trash routes, and shows on **Settings → Trash**.
- `MUSICDROP_TRASH_ORIGINS_DIR` (default `<beets dir>/trash-origins`) — the restore records (see **Backup & restore**); needs a folder of its own, refused under the same rule as the Trash and overlapping the Trash either way.

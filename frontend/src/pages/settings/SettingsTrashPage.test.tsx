...
/** The fifth arm: loose files MusicDrop moved aside to replace them — a
 * portrait `Reset to auto` removed, or art a `Save art to library` run
 * overwrote. Verbatim from `trash_manage._MOVED_ASIDE_NOTE` and, like the two
 * notes above, a hand-kept COPY that no schema can check: refresh it by hand
 * when the backend rewords it. The shape is what the mover produces — two
...
describe("SettingsTrashPage", () => {
  ...
  test("a refused row disables its own controls and nothing else's", async () => {
    ...
    // three single-row tests that each only ever look at their own arm. The
    // moved-aside row rides along for the same reason: it is the one arm whose
    // Restore is absent rather than disabled.
    server.use(
      http.get(TRASH_URL, () =>
        HttpResponse.json({
          albums: [album, importedAlbum, refusedAlbum, movedAsideAlbum],
          trash_path: "/t",
        }),
      ),
    );
    ...
    // Titled by its folder alone now — nothing was read from its tags, so
    // there is no artist to name.
    const refused = controls(/^Symlinked Album$/);
    ...
    const movedAside = rowOf(/^ABBA - artist image$/);
    expect(
      within(movedAside).queryByRole("button", { name: /^Restore$/ }),
    ).toBeNull();
    expect(
      within(movedAside).getByRole("button", { name: /^Empty / }),
    ).toBeEnabled();
  });
  ...
  test("a moved-aside row keeps Empty, renders no Restore, and says why", async () => {
    // `restore_album` short-circuits a `moved="files"` entry to
    // `could_not_restore` without running an import — there is no restore to
    // offer at any point — while both Empty routes reach it normally. So this
    // is the one arm where the button is absent rather than disabled, and the
    // backend's note is what tells the user how to put a file back.
    server.use(
      http.get(TRASH_URL, () =>
        HttpResponse.json({ albums: [movedAsideAlbum], trash_path: "/t" }),
      ),
    );
    renderPage();

    const empty = await screen.findByRole("button", {
      name: /Empty ABBA - artist image/i,
    });
    expect(empty).toBeEnabled();
    expect(screen.queryByRole("button", { name: /^Restore$/ })).toBeNull();
    expect(screen.getByText("Files moved aside.")).toBeInTheDocument();
    expect(screen.getByText(MOVED_ASIDE_NOTE)).toBeInTheDocument();
    expect(screen.getByText("/data/cache/artist-images")).toBeInTheDocument();
    // The labels of the arms that DO have a restore must not appear here: one
    // promises an approximate one, the other describes a disabled button.
    expect(screen.queryByText("Approximate restore.")).not.toBeInTheDocument();
    expect(screen.queryByText("Can’t be restored.")).not.toBeInTheDocument();
    // The zero-track hedge hints at a Restore this row does not have.
    expect(
      screen.queryByText(/couldn.t read audio tags here/i),
    ).not.toBeInTheDocument();
    // The note is the only control-level explanation left, so it has to reach
  ...
  test("a row with no artist and no album is titled by its folder alone", async () => {
    // "Unknown artist - ABBA - artist image" invented an artist for an entry
    // that never had one. Both null means nothing was read from tags, so the
    // folder IS the name — and it must appear ONCE: the subtitle used to fall
    // back to the same folder, printing it twice on the very rows this fixes.
    server.use(
      http.get(TRASH_URL, () =>
        HttpResponse.json({ albums: [movedAsideAlbum], trash_path: "/t" }),
      ),
    ...
    expect(
      await screen.findByText("Files moved aside."),
    ).toBeInTheDocument();
    expect(screen.getAllByText("ABBA - artist image")).toHaveLength(1);
    expect(screen.queryByText(/Unknown artist/)).not.toBeInTheDocument();
  });

  test("the moved-aside confirm does not call these files an album", async () => {
    let deleted: string | null = null;
    server.use(
      http.get(TRASH_URL, () =>
        HttpResponse.json({ albums: [movedAsideAlbum], trash_path: "/t" }),
    ...
    await user.click(
      await screen.findByRole("button", { name: /Empty ABBA - artist image/i }),
    );
    const dialog = await screen.findByRole("alertdialog");
    expect(
    ...
    await user.click(within(dialog).getByRole("button", { name: "Delete" }));
    await waitFor(() => expect(deleted).toBe("ABBA - artist image"));
  });

  test("an album's own confirm still says whose files it deletes", async () => {
    // Control for the wording above: with one string for both, either test
    // passes on the other row's sentence.
    server.use(
      http.get(TRASH_URL, () =>
        HttpResponse.json({ albums: [album], trash_path: "/t" }),
      ),

...
import { apiFetch, errorDetail } from "@/api/lib";
...
export function useFetchAlbumCover(albumId: number) {
  return useMutation<FetchedCover, Error, void>({
    mutationFn: async () => {
      const res = await apiFetch(`/api/albums/${albumId}/cover/fetch`, { method: "POST" });
...
export function useInstallAlbumCover(albumId: number) {
  ...
  return useMutation<CoverInstallResult, Error, Blob>({
    mutationFn: async (image) => {
      ...
      const res = await apiFetch(`/api/albums/${albumId}/cover`, { method: "POST", body: form });

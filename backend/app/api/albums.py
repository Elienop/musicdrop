...
from app.models.lyrics import AlbumLyricsResult
...
@router.post("/albums/{album_id}/lyrics/fetch", response_model=AlbumLyricsResult)
async def fetch_album_lyrics_endpoint(
    album_id: int,
    request: Request,
    handle: Annotated[LibraryHandle, Depends(get_library)],
) -> AlbumLyricsResult:
    """Fetch missing lyrics for an album's tracks into the files (Plex reads them).

    Skip-existing; per-track outcomes + counts. 404 unknown album, 409 while importing.
    """
    return await fetch_album_lyrics_op(request, album_id)

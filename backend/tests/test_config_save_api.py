...
Covers the full Layer-3 save flow: parse, schema-validate, mtime+SHA-256 CAS,
secret preserve, atomic write. The CAS branch is exercised twice — once via a
stale ``mtime_ns`` and once via a hand-forced same-mtime/different-sha to prove
the SHA-256 tie-breaker actually catches edits ``os.utime`` would mask.
"""
...
def test_save_409_on_mtime_change(client: TestClient, beets_library_config_path: Path) -> None:
    mtime, sha = _cas(client)
    time.sleep(0.01)
    future = time.time() + 5
    os.utime(beets_library_config_path, (future, future))
    r = client.post(
        "/api/config/save",
        json={
            "yaml_text": beets_library_config_path.read_text(),
...
def test_save_409_on_sha_change_same_mtime(
    ...
    # Modify content but force the SAME mtime to test the SHA-256 tie-breaker.
